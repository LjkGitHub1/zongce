  ...
  <div class="form-page">
    <el-card>
      <template #header>
        <div class="card-header">
          <div class="header-content">
            ...
            <span class="page-title">{{ isEdit ? '编辑' : '添加' }}专著</span>
          </div>
        </div>
      </template>
      ...
      <el-form
        ...
        class="material-form"
      >
        <el-form-item label="专著名称" prop="title">
          <el-input v-model="form.title" placeholder="请输入专著名称" />
        </el-form-item>
...
const rules = {
  title: [{ required: true, message: '请输入专著名称', trigger: 'blur' }],
  files: [
    { 
      required: true, 
  ...
  await formRef.value.validate(async (valid) => {
    if (valid) {
      ...
      try {
        let material
        if (isEdit.value) {
          const response = await updateMonograph(route.params.id, form)
          material = response.data
        } else {
          const response = await createMonograph(form)
          material = response.data
        }
        ...
        if (filesToUpload.value.length > 0) {
          try {
            await uploadMonographFiles(material.id, filesToUpload.value)
          } catch (uploadError) {
...
onMounted(async () => {
  ...
  if (isEdit.value) {
    try {
      const response = await getMonographs({ id: route.params.id })
