      ...
      <template #header>
        <div class="card-header">
          <div class="header-content">
            ...
            <span class="page-title">{{ isEdit ? '编辑' : '添加' }}其他学术成果</span>
          </div>
        </div>
      </template>
  ...
  await formRef.value.validate(async (valid) => {
    if (valid) {
      ...
      try {
        let material
        if (isEdit.value) {
          const response = await updateOtherAcademic(route.params.id, form)
          material = response.data
        } else {
          const response = await createOtherAcademic(form)
          material = response.data
        }
        ...
        if (filesToUpload.value.length > 0) {
          try {
            await uploadOtherAcademicFiles(material.id, filesToUpload.value)
          } catch (uploadError) {
...
onMounted(async () => {
  ...
  if (isEdit.value) {
    try {
      const response = await getOtherAcademics({ id: route.params.id })
