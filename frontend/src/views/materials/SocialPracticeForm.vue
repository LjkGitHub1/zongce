      ...
      <template #header>
        <div class="card-header">
          <div class="header-content">
            ...
            <span class="page-title">{{ isEdit ? '编辑' : '添加' }}社会实践调研</span>
          </div>
        </div>
      </template>
  ...
  await formRef.value.validate(async (valid) => {
    if (valid) {
      ...
      try {
        let material
        if (isEdit.value) {
          const response = await updateSocialPractice(route.params.id, form)
          material = response.data
        } else {
          const response = await createSocialPractice(form)
          material = response.data
        }
        ...
        if (filesToUpload.value.length > 0) {
          try {
            await uploadSocialPracticeFiles(material.id, filesToUpload.value)
          } catch (uploadError) {
...
onMounted(async () => {
  ...
  if (isEdit.value) {
    try {
      const response = await getSocialPractices({ id: route.params.id })
