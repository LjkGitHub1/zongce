      ...
      <template #header>
        <div class="card-header">
          <div class="header-content">
            ...
            <span class="page-title">{{ isEdit ? '编辑' : '添加' }}服务社会</span>
          </div>
        </div>
      </template>
  ...
  await formRef.value.validate(async (valid) => {
    if (valid) {
      ...
      try {
        let material
        if (isEdit.value) {
          const response = await updateSocialService(route.params.id, form)
          material = response.data
        } else {
          const response = await createSocialService(form)
          material = response.data
        }
        ...
        if (filesToUpload.value.length > 0) {
          try {
            await uploadSocialServiceFiles(material.id, filesToUpload.value)
          } catch (uploadError) {
...
onMounted(async () => {
  ...
  if (isEdit.value) {
    try {
      const response = await getSocialServices({ id: route.params.id })
